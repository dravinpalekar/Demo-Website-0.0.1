...
import dravin.com.repository.repository.specification.PermissionEntitySearchSpecification;
...
public class PermissionService {
    ...
    public ResponseEntity<Map<String, String>> createPermission(CreatePermissionRequestModel requestObject) {
        ...
        Optional<PermissionEntity> permissionEntity = permissionRepository
                .findByNameAndDeletedAtIsNull(requestObject.getName());
    ...
    public ResponseEntity<Map<String, String>> updatePermissionById(Long id,
            CreatePermissionRequestModel requestObject) {
        return permissionRepository.findByIdAndDeletedAtIsNull(id)
                ...
                .orElseGet(
                        () -> ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of(MESSAGE, PERMISSION_NOT_FOUND)));
    }

    public ResponseEntity<Map<String, Object>> getAllPermission(Pageable pageable, String searchItem) {

        Specification<PermissionEntity> specification = PermissionEntitySearchSpecification.notDeleted()
                .and(PermissionEntitySearchSpecification.search(searchItem));
        Page<PermissionEntity> permissionEntities = permissionRepository.findAll(specification, pageable);
        return ResponseEntity.ok(Map.of(
                "data", permissionEntities.getContent(),
                "pageSize", permissionEntities.getSize(),
                "getTotalElements", permissionEntities.getTotalElements()));
    }

    public ResponseEntity<Map<String, Object>> getPermissionById(Long id) {

        return ResponseEntity.ok(Map.of("data", permissionRepository.findByIdAndDeletedAtIsNull(id)
        ...
        if (permissionEntity.isPresent()) {
            if (roleRepository.existsByPermissionId(id)) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error",
                        "Cannot delete permission because it is currently assigned to one or more roles."));
            } else {
