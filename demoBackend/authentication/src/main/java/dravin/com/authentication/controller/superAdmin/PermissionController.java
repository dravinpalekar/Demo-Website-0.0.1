...
public class PermissionController {
    ...
    @PostMapping(CREATE)
    @Operation(summary = "Create permissions.", description = "This API is for creating permissions.")
    public ResponseEntity<Map<String, String>> createPermission(
            @Valid @RequestBody CreatePermissionRequestModel createPermissionRequest) {

        return permissionService.createPermission(createPermissionRequest);
    }

    @PutMapping(UPDATE + ID)
    @Operation(summary = "Update permission by id", description = "This API is for updating permission by id.")
    public ResponseEntity<Map<String, String>> updatePermissionById(@PathVariable Long id,
            @Valid @RequestBody CreatePermissionRequestModel createPermissionRequest) {
    ...
    @GetMapping(GET)
    @Operation(summary = "Get all permissions of users", description = "This API is for getting all user permissions.")
    public ResponseEntity<Map<String, Object>> getAllPermission(@RequestParam(required = false) String searchItem,
            @PageableDefault(page = 0, size = 10, sort = "id", direction = Sort.Direction.DESC) Pageable pageable) {

        return permissionService.getAllPermission(pageable, searchItem);
    }

    @GetMapping(GET + ID)
    @Operation(summary = "Get permission by id", description = "This API is for getting a user permission by ID.")
    ...
    @Operation(summary = "Delete permission by id", description = "This API is for deleting a user permission by ID.")
